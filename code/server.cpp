...
class MntResource : public Resource
{
    ...
    private:
        ...
        int m_var_value_err; // the value for the attribute "err": last HTTP occurred error
        std::string m_var_name_err = "err"; // the name for the attribute "err" 
    ...
    m_cv{}
{
    ...
    m_var_value_rt.push_back("oic.wk.mnt");m_var_value_n = "";  // current value of property "n" Friendly name of the resource
    // initialize vector if  The interface set supported by this resource
    
    m_var_value_if.push_back("oic.if.baseline");
    m_var_value_if.push_back("oic.if.rw");m_var_value_rb = false; // current value of property "rb" Reboot Action
    m_var_value_err = 503; // current value of property "err" last HTTP occurred error
    EntityHandler cb = std::bind(&MntResource::entityHandler, this,PH::_1);
    OCStackResult result = OCPlatform::registerResource(m_resourceHandle,
                                                        resourceURI,
                                                        m_RESOURCE_TYPE[0],
                                                        m_RESOURCE_INTERFACE[0],
...
OCRepresentation MntResource::get(QueryParamsMap queries)
{
    ...
    m_rep.setValue(m_var_name_n, m_var_value_n ); 
    m_rep.setValue(m_var_name_if,  m_var_value_if ); 
    std::cout << "\t\t" << "property 'rb' : "<< m_var_value_rb << std::endl;
    m_rep.setValue(m_var_name_rb, m_var_value_rb ); 
    std::cout << "\t\t" << "property 'err' : "<< m_var_value_err << std::endl;
    m_rep.setValue(m_var_name_err, m_var_value_err ); 
...
OCEntityHandlerResult MntResource::post(QueryParamsMap queries, const OCRepresentation& rep)
{
    ...
    try {
        if (rep.hasAttribute(m_var_name_err))
        {
            // value exist in payload
            // allocate the variable
            int value;
            // get the actual value from the payload
            rep.getValue(m_var_name_err, value);
            
            // check if "err" is read only
            ehResult = OC_EH_ERROR;
            std::cout << "\t\t" << "property 'err' is readOnly "<< std::endl;
            
            if ( value < 399 )
            {
                // check the minimum range
                std::cout << "\t\t" << "property 'err' value smaller than minimum :" << 399 << " value: " << value << std::endl;
                ehResult = OC_EH_ERROR;
            }
            if ( value > 599 )
            {
                // check the maximum range
                std::cout << "\t\t" << "property 'err' value exceed max :" << 599 << " value: " <<value << std::endl;
                ehResult = OC_EH_ERROR;
            }
        }
    }
    ...
    if (ehResult == OC_EH_OK)
    {
        ...
        try {
            bool temp;
            if (rep.getValue(m_var_name_rb, temp ))
            {
                m_var_value_rb = temp;
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_err, temp ))
            {
                m_var_value_err = temp;
                std::cout << "\t\t" << "property 'err' UPDATED: " << m_var_value_err << std::endl;
                m_send_notification_flag = true;
            }
            else
            {
                std::cout << "\t\t" << "property 'err' not found in the representation" << std::endl;
            }
        }
        catch (std::exception& e)
        {
    ...
    m_cv{}
{
    ...
    m_var_value_rt.push_back("oic.wk.nmon");m_var_value_tx = 10; // current value of property "tx" Amount of transmitted kilo bytes from collection start time 'time'
    m_var_value_rx = 15; // current value of property "rx" Amount of received kilo bytes from collection start time 'time'
    m_var_value_amstx = 35; // current value of property "amstx" Average transmitted message size in bytes (tx) in the collection period
    m_var_value_col = false; // current value of property "col" True: Device is collecting values
    // initialize vector if  The interface set supported by this resource
    
    m_var_value_if.push_back("oic.if.baseline");
    m_var_value_if.push_back("oic.if.rw");m_var_value_mmstx = 50; // current value of property "mmstx" Maximum transmitted message size in bytes (tx) in the collection period
    m_var_value_reset = false; // current value of property "reset" True: reset the collected values
    m_var_value_mmsrx = 35; // current value of property "mmsrx" Maximum received message size in bytes (rx) in the collection period
...
OCRepresentation NmonResource::get(QueryParamsMap queries)
{
    ...
    std::cout << "\t\t" << "property 'rx' : "<< m_var_value_rx << std::endl;
    m_rep.setValue(m_var_name_rx, m_var_value_rx ); 
    std::cout << "\t\t" << "property 'amstx' : "<< m_var_value_amstx << std::endl;
    m_rep.setValue(m_var_name_amstx, m_var_value_amstx ); 
    std::cout << "\t\t" << "property 'col' : "<< m_var_value_col << std::endl;
    m_rep.setValue(m_var_name_col, m_var_value_col ); 
    m_rep.setValue(m_var_name_if,  m_var_value_if ); 
    std::cout << "\t\t" << "property 'mmstx' : "<< m_var_value_mmstx << std::endl;
    m_rep.setValue(m_var_name_mmstx, m_var_value_mmstx ); 
    std::cout << "\t\t" << "property 'reset' : "<< m_var_value_reset << std::endl;
    m_rep.setValue(m_var_name_reset, m_var_value_reset ); 
    std::cout << "\t\t" << "property 'mmsrx' : "<< m_var_value_mmsrx << std::endl;
    m_rep.setValue(m_var_name_mmsrx, m_var_value_mmsrx ); 
    std::cout << "\t\t" << "property 'amsrx' : "<< m_var_value_amsrx << std::endl;
    m_rep.setValue(m_var_name_amsrx, m_var_value_amsrx ); 
    std::cout << "\t\t" << "property 'ianaifType' : "<< m_var_value_ianaifType << std::endl;
...
OCEntityHandlerResult NmonResource::post(QueryParamsMap queries, const OCRepresentation& rep)
{
    ...
    if (ehResult == OC_EH_OK)
    {
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_tx, temp ))
            {
                m_var_value_tx = temp;
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_rx, temp ))
            {
                m_var_value_rx = temp;
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_amstx, temp ))
            {
                m_var_value_amstx = temp;
        ...
        try {
            bool temp;
            if (rep.getValue(m_var_name_col, temp ))
            {
                m_var_value_col = temp;
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_mmstx, temp ))
            {
                m_var_value_mmstx = temp;
        ...
        try {
            bool temp;
            if (rep.getValue(m_var_name_reset, temp ))
            {
                m_var_value_reset = temp;
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_mmsrx, temp ))
            {
                m_var_value_mmsrx = temp;
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_amsrx, temp ))
            {
                m_var_value_amsrx = temp;
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_ianaifType, temp ))
            {
                m_var_value_ianaifType = temp;
